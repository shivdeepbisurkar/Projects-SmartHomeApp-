...
public class Utilities extends HttpServlet {
	 ...
	 * If the html file name is Header.html then It gets Username from session
	 * variables.
			...
			if (session.getAttribute("username") != null) {
				...
				if (session.getAttribute("usertype").equals("manager")) {
					result = result
							...
							+ "<li><a href='SaleReport'><span class='glyphicon'>SaleReport</span></a></li>"
					...
					result = result
							+ "<li><a href='Registration'><span class='glyphicon'>Create Customer</span></a></li>"
	...
	public void storeProduct(String name, String type, String maker, String acc, String discount) {
		if (!OrdersHashMap.orders.containsKey(username())) {
		...
		HashMap<String, Thermo> allthermos = new HashMap<String, Thermo>();
		HashMap<String, Light> alllights = new HashMap<String, Light>();
		...
		if (type.equals("consoles")) {
			...
			OrderItem orderitem = new OrderItem(console.getName(), console.getPrice(), console.getImage(),
					console.getRetailer(), type, discount);
		...
		if (type.equals("tablets")) {
			...
			OrderItem orderitem = new OrderItem(tablet.getName(), tablet.getPrice(), tablet.getImage(),
					tablet.getRetailer(), type, discount);
			orderItems.add(orderitem);
		}

		if (type.equals("thermos")) {
			Thermo thermo = null;
			try {
				allthermos = MySqlDataStoreUtilities.getThermos();
			} catch (Exception e) {

			}
			thermo = allthermos.get(name);
			OrderItem orderitem = new OrderItem(thermo.getName(), thermo.getPrice(), thermo.getImage(),
					thermo.getRetailer(), type, discount);
			orderItems.add(orderitem);
		}
		if (type.equals("lights")) {
			...
			OrderItem orderitem = new OrderItem(light.getName(), light.getPrice(), light.getImage(),
					light.getRetailer(), type, discount);
			orderItems.add(orderitem);
		}
		if (type.equals("accessories")) {
			...
			OrderItem orderitem = new OrderItem(accessory.getName(), accessory.getPrice(), accessory.getImage(),
					accessory.getRetailer(), type, discount);
			orderItems.add(orderitem);
		}
	...
	public void storePayment(int orderId,
			...
			String purchasedate, String shippingdate, String quantity, String deliveryType, String zip,
			String address) {
		HashMap<Integer, ArrayList<OrderPayment>> orderPayments = new HashMap<Integer, ArrayList<OrderPayment>>();
		...
		OrderPayment orderpayment = new OrderPayment(orderId, username(), orderName, orderPrice, userAddress,
				creditCardNo, purchasedate, shippingdate, deliveryType, address);
		listOrderPayment.add(orderpayment);

		// add order details into database
		try {
			if (session.getAttribute("usertype").equals("retailer")) {
				MySqlDataStoreUtilities.insertOrder(orderId, customer, orderName, orderPrice, userAddress,
						creditCardNo, purchasedate, shippingdate, quantity, deliveryType, zip, address);
			} else

			{
				MySqlDataStoreUtilities.insertOrder(orderId, username(), orderName, orderPrice, userAddress,
						creditCardNo, purchasedate, shippingdate, quantity, deliveryType, zip, address);
			}
		} catch (Exception e) {
	...
	public String storeReview(String productname, String producttype, String productprice, String storeid,
			String zipcode, String retailercity,
		...
		String message = MongoDBDataStoreUtilities.insertReview(productname, username(), producttype, productprice,
				storeid, zipcode, retailercity,
		...
		if (!message.equals("Successfull")) {
			...
			Review review = new Review(productname, username(), producttype, productprice,
					storeid, zipcode, retailercity,
					storeState, productOnSale, productmaker, manufacturerRebate, userid, userage, usergender,
					useroccupation, reviewrating, reviewdate, reviewtext);
	...
	public HashMap<String, Thermo> getThermos() {
		HashMap<String, Thermo> hm = new HashMap<String, Thermo>();
		hm.putAll(SaxParserDataStore.thermos);
		return hm;
	}
	...
	public ArrayList<String> getProductsThermos() {
		ArrayList<String> ar = new ArrayList<String>();
		for (Map.Entry<String, Thermo> entry : getThermos().entrySet()) {
			ar.add(entry.getValue().getName());
		}
		return ar;
	}
	...
	public ArrayList<String> getProductsLights() {
		ArrayList<String> ar = new ArrayList<String>();
		for (Map.Entry<String, Light> entry : getLights().entrySet()) {
			ar.add(entry.getValue().getName());
		}
		return ar;
	}
