...
public class ProductCrud extends HttpServlet {
	protected void doGet(HttpServletRequest request,
		...
		String producttype = "", productId = "", productName = "", productImage = "", productManufacturer = "",
				productCondition = "", prod = "", rebat = "";
		double productPrice = 0.0, productDiscount = 0.0, count = 0.0;
		...
		if (action.equals("add") || action.equals("update")) {
			...
			rebat = request.getParameter("yesno");
		} else {
			productId = request.getParameter("productId");
		}
			...
			if (msg.equals("good")) {
				try {
					msg = MySqlDataStoreUtilities.addproducts(producttype, productId, productName, productPrice,
							productImage, productManufacturer, productCondition, productDiscount, prod, count, rebat);
				} catch (Exception e) {
			...
			if (msg.equals("good")) {
				...
				try {
					msg = MySqlDataStoreUtilities.updateproducts(producttype, productId, productName, productPrice,
							productImage, productManufacturer, productCondition, productDiscount, count,rebat);
				} catch (Exception e) {
